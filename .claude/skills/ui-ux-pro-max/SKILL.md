...
The search script lives inside this skill's own directory, not the project directory. Always invoke it by its full path — do not assume a particular working directory. This copy was installed as a **project skill** (not a marketplace plugin), so paths below are relative to the repository root, not `${CLAUDE_PLUGIN_ROOT}`:
